...
class ProteinFoldingEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        if self.state in self.path:
            ...
            self.new_path = self.path.copy()
            self.new_path.append(self.state)
            if(abs(self.calculate_energy(self.path)-self.calculate_energy(self.new_path))) == 0 :
                reward = 0
            else:
                reward = 1/(abs(self.calculate_energy(self.path)-self.calculate_energy(self.new_path)))

        if self.current_index == self.protein_length - 1:
            done = True
